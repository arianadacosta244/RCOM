...
//   Eduardo Nuno Almeida [enalmeida@fe.up.pt]
...
int init_ring_buffers(void)
{
    ...
    memset(par.tx2rxValid, 0, par.bufSize);
    memset(par.rx2txValid, 0, par.bufSize);
    par.tx2rxIdx = 0;
    par.rx2txIdx = 0;
...
void set_rt_priority(void)
{
    ...
    if (pthread_setschedparam(self, SCHED_FIFO, &sp))
        return;
    ...
    if (setpriority(PRIO_PROCESS, 0, -10) < 0)
        perror("Could not set realtime-like priority on macOS: %s\n");
